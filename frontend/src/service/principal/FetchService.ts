 ...
 */
async function initHeaders(withToken: boolean, customHeaders: Array<HeaderPair> = []): Promise<Headers> {
    const headers: HeadersInit = new Headers();
    headers.set("Accept", "application/json");
    headers.set("Content-Type", "application/json");
    customHeaders.forEach(h => headers.set(h.key, h.value));
    if (withToken) headers.set("Authorization", "Bearer " + principal.value.token.token);
    return headers;
}
 ...
 */
async function wrappedFetch(url: string, method: string, body = "", authentication = true, customHeaders: Array<HeaderPair> = []): Promise<Response> {
    const fetchOptions: FetchOptions = { method: method, headers: await initHeaders(authentication, customHeaders) };
    if (body !== "") fetchOptions["body"] = body;
    const response = await fetch(url, fetchOptions);
    if (response.status == 200) {
        const tokenHeader = response.headers.get("X-Authorization");
        if (tokenHeader !== null) {
            const newToken: Token = { token: tokenHeader.substring("Bearer ".length) };
            setToken(newToken);
        }
    } else {
